...
const EventsCard = ({imag, eventDate, eventName, eventDescription}) => {
    return (
        <motion.div
            className={'min-w-[60%] h-full rounded-[40px] select-none relative'}
        >
            <Image src={imag} alt={''} className={'select-none w-full h-full rounded-[40px]'}/>
            <div className={'absolute rounded-[40px] top-0 h-full w-full backdrop-blur-[2px] hover:backdrop-blur-0 duration-500 transition-all'}>
            </div>
